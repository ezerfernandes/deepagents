...
def get_provider_auth_status(provider: str) -> ProviderAuthStatus:
    ...
    if env_var:
        ...
        # needed when the model is created.
        if _has_oauth_credentials(provider):
            return ProviderAuthStatus(
                state=ProviderAuthState.CONFIGURED,
                provider=provider,
                detail="OAuth credentials stored",
            )
    ...
    # OAuth-only providers may not have a hardcoded env-var mapping. Keep the
    # check outside the env-var branch so those logins still surface as ready.
    if _has_oauth_credentials(provider):
        return ProviderAuthStatus(
            state=ProviderAuthState.CONFIGURED,
            provider=provider,
